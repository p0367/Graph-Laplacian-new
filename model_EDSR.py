...
def edsr(input, scale, num_filters=64, num_res_blocks=8, res_block_scaling=None):
    x = input - 0.5
    #x = input
    x = b = tf.layers.conv2d(x, num_filters, 3, padding='same', activation=None)
    for i in range(num_res_blocks):
        b = res_block(b, num_filters)
    b = tf.layers.conv2d(b, num_filters, 3, padding='same', activation=None)
    x = x + b

    x = upsample(x, scale, num_filters)
    x = tf.layers.conv2d(x, 3, 3, padding='same', activation=None)
    x = x + 0.5

    return x ,x

def res_block(x_in, filters):
    x = tf.layers.conv2d(x_in, filters, 3, padding='same', activation=tf.nn.relu)
...
def upsample(x, scale, num_filters):
    def upsample_1(x, factor, **kwargs):
        x = tf.layers.conv2d(x, num_filters * (factor ** 2), 3, padding='same', activation=None)
        return x
...
class denoiser(object):
    ...
    def __init__(self, sess, input_c_dim=3, sigma=25, batch_size=batch_size):
        ...
        self.Y = edsr(self.X, scale, num_filters=64, num_res_blocks=8, res_block_scaling=None)

        #self.Y_b = tf.image.resize(self.X, (output_size,output_size), method='bicubic')
        self.Y_b = self.X
        ##ground
        self.Y_2 = self.Y_   #for evaluate
        #loss
        ####graph
        self.Y0 = self.Y[0]
        ...
        #self.loss2 = tf.norm(self.loss20,ord=1) * (1 / (batch_size*output_size*output_size*channel))
        ...
        #self.loss13 = tf.math.abs(self.loss12)
