...
def wdsr(input,scale, num_filters=32, num_res_blocks=8, res_block_expansion=4, res_block_scaling=None):
    x_in = input
    x = x_in
    ...
    for i in range(num_res_blocks):
        m = res_block_a(m, num_filters, res_block_expansion, kernel_size=3, scaling=res_block_scaling)
    m = conv2d_weightnorm(3 * scale ** 2, 3, padding='same', name=f'conv2d_main_scale_{scale}')(m)
    #m = Lambda(pixel_shuffle(scale))(m)
    #m = tf.nn.depth_to_space(m, scale)

    # skip branch
    s = conv2d_weightnorm(3 * scale ** 2, 5, padding='same', name=f'conv2d_skip_scale_{scale}')(x)
    #s = Lambda(pixel_shuffle(scale))(s)
    #s = tf.nn.depth_to_space(s, scale)

    x = m + s
    
    x = Conv2D(3 ,3, padding='same', activation=None)(x)

    #x = conv2d_weightnorm(3, 3, padding='same', name=f'conv2d_skip_scale_{scale}')(x)
...
def conv2d_weightnorm(filters, kernel_size, padding='same', activation=None, **kwargs):
    ...
    #return tfa.layers.WeightNormalization(tf.layers.conv2d(x, filters, kernel_size, padding='same', activation=None))
...
class denoiser(object):
    ...
    def __init__(self, sess, input_c_dim=3, sigma=25, batch_size=batch_size):
        ...
        self.Y = wdsr(self.X,scale, num_filters=32, num_res_blocks=8, res_block_expansion=4, res_block_scaling=None)
